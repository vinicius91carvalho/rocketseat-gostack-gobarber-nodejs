...
describe('ResetPassword', () => {
    ...
    it('should be able to reset the password', async () => {
        ...
        expect(generateHash).toHaveBeenLastCalledWith(newPassword);
        expect(updatedUser?.password).toBe(newPassword);
    });

    it('should not be able to reset the password with non-existing token', async () => {
        await expect(resetPassword.execute({ token: 'non-existing-token', password: '123456' })).rejects.toBeInstanceOf(
            AppError,
        );
    });
    ...
    it('should not be able to reset the password if passed more than 2 hours', async () => {
        ...
        await expect(resetPassword.execute({ token, password: newPassword })).rejects.toBeInstanceOf(AppError);
    });
});
